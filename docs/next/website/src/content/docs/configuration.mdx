...
The default keymap is prefix-first so Herdr does not steal input from shells, editors, tmux, or terminal apps. In workspace navigation, `Enter` opens the selected workspace; `navigate_workspace_open` adds an optional direct-key alias such as `o`. Custom commands are not run while the workspace navigation preview is active. Search `keys.` in the [Config reference](/docs/config-reference/) to see every action and default binding. The in-app help panel at `prefix+?` shows the active bindings.
...
Destructive confirmation dialogs (close workspace/tab/pane, delete worktree checkout) always accept `Enter` and cancel with `esc`. The `confirm_accept` field is dialog-only and adds an optional direct-key alias such as `y` for tmux habit; it is unset by default and must not use `prefix+` or `esc`:
...
percentage of the full terminal area. Percentages resolve against the entire
terminal (including the sidebar and tab bar) and the popup is centered over
it, matching tmux `display-popup -w/-h`. Dimensions include the popup border, and
...
Set `tab_bar_numbers = true` under `[ui]` to show the current position before custom-named desktop tabs. Agent status indicators remain before the number, for example `● 2 logs`. The default is `false`.

Set `sidebar_position = "right"` under `[ui]` to place the desktop sidebar on the right side of the terminal panes. The default is `"left"`.
...
- `state_elapsed` — time since the displayed status changed, rounded down to minutes.
